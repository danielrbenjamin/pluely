...
export const useCompletion = () => {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = async (e: StorageEvent) => {
      if (e.key === "friday-conversation-selected" && e.newValue) {
        try {
        ...
        if (!hasPermission) {
          ...
          if (!hasPermissionNow) {
            setState((prev) => ({
              ...prev,
              error:
                "Screen Recording permission required. Please enable it by going to System Settings > Privacy & Security > Screen & System Audio Recording. If you don't see FRIDAY in the list, click the '+' button to add it. If it's already listed, make sure it's enabled. Then restart the app.",
            }));
